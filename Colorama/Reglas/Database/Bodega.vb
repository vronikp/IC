'------------------------------------------------------------------------------
' <auto-generated>
'     This code was generated by a tool.
'     Runtime Version:2.0.50727.42
'
'     Changes to this file may cause incorrect behavior and will be lost if
'     the code is regenerated.
' </auto-generated>
'------------------------------------------------------------------------------

Option Strict Off
Option Explicit On

Imports System
Imports System.Xml
Imports System.Xml.Serialization
Imports Infoware.Reglas

#Region "Bodega"
Partial Public Class Bodega
  Inherits _Database

  Private mEmpres_Codigo As Integer = 0

  Private mSucurs_Codigo As Integer = 0

  Private mBodega_Codigo As Integer = 0

  Private mEntida_Codigo As Integer = 0

  Private mBodega_Visible As Boolean = True

  Private mBodega_principal As Boolean = False

  Public Sub New()
    MyBase.New()
  End Sub

  <XmlAttribute()> _
  Public Overridable Property Empres_Codigo() As Integer
    Get
      Return Me.mEmpres_Codigo
    End Get
    Set(ByVal value As Integer)
      Me.mEmpres_Codigo = value
      EsModificado = True
    End Set
  End Property

  <XmlAttribute()> _
  Public Overridable Property Sucurs_Codigo() As Integer
    Get
      Return Me.mSucurs_Codigo
    End Get
    Set(ByVal value As Integer)
      Me.mSucurs_Codigo = value
      EsModificado = True
    End Set
  End Property

  <XmlAttribute()> _
  Public Overridable Property Bodega_Codigo() As Integer
    Get
      Return Me.mBodega_Codigo
    End Get
    Set(ByVal value As Integer)
      Me.mBodega_Codigo = value
      EsModificado = True
    End Set
  End Property

  <XmlAttribute()> _
  Public Overridable Property Entida_Codigo() As Integer
    Get
      Return Me.mEntida_Codigo
    End Get
    Set(ByVal value As Integer)
      Me.mEntida_Codigo = value
      EsModificado = True
    End Set
  End Property

  <XmlAttribute()> _
  Public Overridable Property Bodega_Visible() As Boolean
    Get
      Return Me.mBodega_Visible
    End Get
    Set(ByVal value As Boolean)
      Me.mBodega_Visible = value
      EsModificado = True
    End Set
  End Property

  <XmlAttribute()> _
  <Infoware.Reportes.CampoReporteAtributo("Principal", Infoware.Reportes.CampoReporteAtributo.EnumTipoDato.Logico, 110, True)> _
  Public Overridable Property Bodega_principal() As Boolean
    Get
      Return Me.mBodega_principal
    End Get
    Set(ByVal value As Boolean)
      Me.mBodega_principal = value
      EsModificado = True
    End Set
  End Property
End Class
#End Region
